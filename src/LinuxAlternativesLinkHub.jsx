...
export default function LinuxAlternativesLinkHub() {
  ...
  const technicalItems = [
    {
      name: "Pi-hole",
      blurb: "Network-wide ad and tracker blocking that can help clean up browsing across devices on your network.",
      bestFor: "Network filtering",
      site: "https://pi-hole.net/",
      accent: "#6ee7b7",
      bg: "from-emerald-600 via-lime-700 to-green-800",
      details: ["Blocks ads network-wide", "Works on any device", "Web dashboard", "Easy Raspberry Pi setup"],
    },
    {
      name: "FSArchiver",
      blurb: "A filesystem backup and restore tool that is useful for image-based recovery and migration workflows.",
      bestFor: "System imaging",
      site: "https://www.fsarchiver.org/",
      accent: "#fcd34d",
      bg: "from-amber-600 via-yellow-700 to-orange-800",
      details: ["Full system imaging", "Compression support", "Cross-filesystem restore", "CLI based"],
    },
    {
      name: "Restic",
      blurb: "A fast, secure backup tool with encryption and support for local and cloud-style storage backends.",
      bestFor: "Encrypted backups",
      site: "https://restic.net/",
      accent: "#7dd3fc",
      bg: "from-cyan-600 via-sky-700 to-blue-800",
      details: ["End-to-end encryption", "Deduplication", "Multiple backends", "Fast and reliable"],
    },
    {
      name: "Performance Tweaks",
      blurb: "Practical tuning ideas: power profiles, zram, fstrim, scheduler choices, and desktop responsiveness tweaks.",
      bestFor: "Speed & responsiveness",
      site: "#technical-performance",
      accent: "#c4b5fd",
      bg: "from-violet-600 via-fuchsia-700 to-indigo-800",
      details: ["Power profile tuning", "zram swap compression", "SSD trim scheduling", "Kernel scheduler options"],
    },
  ];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4" onClick={() => setShowDonate(false)}>
      ...
      <div className="relative w-full max-w-md rounded-3xl border border-white/10 bg-slate-900 p-6 shadow-2xl shadow-black/40" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          ...
          {/* PayPal */}
          <a href="https://paypal.com/donate?business=chuck2045&currency_code=USD" target="_blank" rel="noreferrer"
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-400">Donate via PayPal</p>
            </div>
        ...
        <p className="mt-5 text-center text-xs text-slate-500">No account needed for Cash App. PayPal accepts cards too.</p>
      </div>
    </div>
  );
...
sudo crontab -l`} />
              ...
              <div className="rounded-3xl border border-indigo-400/30 bg-indigo-400/10 p-6">
                <div className="flex items-start gap-4">
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-indigo-400/30 bg-indigo-400/15 text-2xl">🔒</div>
                  <div className="flex-1">
                    <div className="flex items-center gap-3 flex-wrap">
                      <h3 className="text-lg font-semibold text-indigo-200">Pair it with a VPN — Private Internet Access (PIA)</h3>
                    ...
                    <p className="mt-2 text-sm text-slate-300">
                      DNS filtering handles what your machine resolves — a VPN handles what your ISP and the network around you can see. Together they cover both layers. PIA (Private Internet Access) is a solid, long-standing VPN provider with native Linux support, a no-logs policy, and a kill switch. It works well alongside a local dnsmasq sinkhole without conflicts.
                    </p>
                    <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-400">
                      {["Native Linux client", "No-logs policy", "Kill switch included", "Works with local DNS", "Open source client"].map(f => (
                        <span key={f} className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5">{f}</span>
                      ))}
                    </div>
                    <a href="https://www.privateinternetaccess.com" target="_blank" rel="noreferrer"
                      className="mt-4 inline-flex items-center gap-2 rounded-2xl border border-indigo-400/30 bg-indigo-400/10 px-4 py-2 text-sm font-semibold text-indigo-200 transition hover:bg-indigo-400/20">
